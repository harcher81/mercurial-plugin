        ...
        try {
            ...
            try {
                ...
                if (inst.isUseCaches()) {
                    ...
                    if (localCache.isDirectory()) {
                        ArgumentListBuilder args = slaveHg.seed(true).add("pull");
                        ...
                        localCache.mkdirs();
                        ArgumentListBuilder args = slaveHg.seed(true).add("clone").add("--noupdate").add(remote);
                        if (HgExe.joinWithPossibleTimeout(slaveHg.launch(args.add(localCache.getRemote())), useTimeout, listener) != 0) {
                            listener.error("Failed to clone " + remote);
                            return null;
                        }
                    }
